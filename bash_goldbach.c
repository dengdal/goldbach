...
int main()
{
    ...
    while((number < max_number)) {  
        ...
        if(equality_holds==0) {
            printf("Goldbachs L-K conjecture does not hold for number: %ld \n", number);
            found_stern_number += 1;
        }
        number+=2; // Next odd number
        if((number%progress_print_interval)==0) // Indicate progression
            printf(".");
    }
    ...
    if(found_stern_number==0) 
        ...
        printf("\nGoldbachs L-K conjecture does not hold. Found %d Stern numbers\n", found_stern_number);
    printf ("Time consumed: %ld secs.\n\n", time(NULL)-begin);

    return ret;
}

/* Checks if Goldbachs lesser known conjecture holds for number 
 * is_goldbach() uses a primes_field generated by gen_primes
 */
int is_goldbach(long number, unsigned char *primes_field)
{
    ...
    for (i = floor_sqrt_half_n; i >= 0; i--) {
        equality_holds = 0; // We will test if the equality holds for this value of i
        rest = number - 2*i*i;
...
/* Generates a filed of primes up to number
 * Uses seiving.
 * The code was taken from 
 * http://web.archive.org/web/20130519082757/http://www.fpx.de/fp/Software/sieve.c
 * and modified
 * The char array that the pointer primes_field should point at has one bit
 * position for each integer up to number. Each such position is marked 
 * by the gen_primes_field as prime or not.  
 */
int gen_primes(long number, unsigned char *primes_field)
{
    unsigned char *zzz;
    ...
    begin = time (NULL);
    while ((teste+=2) < number)
        if (!TEST(primes_field, teste)) {
            if  (++hits%2000L==0) {printf (" %ld. prime number\x0d", hits); fflush(stdout);}
            for (mom=3L*teste; mom<number; mom+=teste<<1) SET (primes_field, mom);
        }
